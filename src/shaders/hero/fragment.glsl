...
void main(void) {
    ...
    // Depth gradient across two hues: deep blue troughs -> teal sky.
    // The bright end keeps the greenish teal; shadows fall into blue for contrast.
    vec3 waveColor = vec3(0.040, 0.136, 0.360); // hsl(222, 80%, 20%)  deep blue
    vec3 skyColor  = vec3(0.346, 0.802, 0.893); // hsl(190, 72%, 62%)  teal
    float ramp = clamp(t / 50.0, 0.0, 1.0);
    vec3 col = mix(waveColor, skyColor, ramp);

    // Specular highlights pick out the surface form
    col += c.rgb * (SPECULAR_STRENGTH + (iZoomOffset * 0.1)) * vec3(0.40, 0.60, 0.89);
